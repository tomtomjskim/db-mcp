...
### 기본 동작 확인
- [ ] `execute_query` 기본 응답이 compact JSON (Array-of-Arrays)인지
- [ ] `SHOW COLUMNS` 응답이 TSV 텍스트인지
- [ ] LIMIT 없는 SELECT에 자동 LIMIT 50 적용되는지
- [ ] `totalRows` (COUNT)가 자동 LIMIT 시 함께 반환되는지

### 파라미터 테스트
- [ ] `format: "table"` → TSV 출력
- [ ] `format: "compact"` → JSON 출력
- [ ] `format: "minimal"` → 샘플 5행 + 요약
- [ ] `maxRows: 10` → 10행 제한
- [ ] `preview: true` → 3행 샘플 + 총 행 수
- [ ] 기존 `LIMIT 5` 있는 쿼리에 추가 LIMIT 안 붙는지

### schema_check 도구
- [ ] 테이블 스키마 경량 텍스트 반환
- [ ] 동일 테이블 재호출 시 캐시 히트 (0ms)
- [ ] 존재하지 않는 테이블 에러 처리

### 하위 호환성
- [ ] 기존 `execute_query` 호출 (파라미터 없이)이 정상 동작
- [ ] `natural_language_query` 정상 동작
- [ ] `cross_database_query` 정상 동작
- [ ] 에러 응답 형식 유지
